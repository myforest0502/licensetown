"""Pure PT Phase11 J2/J3 candidate ordering from active current-cycle facts."""
...
def build_j2_candidates(
    ...
    *,
    ...
    for field_id, active in active_field_facts.items():
        cross_confident = int(active.get("active_cross_question_confident_wrong_node_count") or 0)
        confident_nodes = int(active.get("active_confident_wrong_repairing_node_count") or 0)
        if not (cross_confident or confident_nodes >= 2):
            continue
        active_repairing = int(active.get("active_evaluable_wrong_repairing_node_count") or 0)
