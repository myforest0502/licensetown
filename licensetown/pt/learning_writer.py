        ...
        for db_attempt in range(2):
            try:
                with database.get_db_connection() as conn:
                    with conn.cursor() as cur:
                        cur.execute(
                            ...
                            """,
                            (
                                self.qualification_id, event_key, user_id, mode,
                                answered_count, correct_count, timestamp, encoded_results,
                            ),
                        )
                        if cur.rowcount != 1:
                            return False
                        for attempt_position, result in attempts:
                            cur.execute(
                                ...
                                """,
                                (
                                    self.qualification_id, event_key, user_id,
                                    result["question_id"], result["knowledge_node_id"], mode,
                                    json.dumps(
                ...
                if db_attempt:
                    raise
                logger.warning(
                    "PT learning writer transient DB connection failure; retrying once: "
                    "event_key=%s",
                    event_key,
                )
        return False
