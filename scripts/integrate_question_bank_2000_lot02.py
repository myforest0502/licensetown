"""Atomically integrate sealed Question Bank 2000 Lot02 as Q1810-Q1857.
...
from reports.question_bank_2000_lot02_validate import (
    STAGING,
    build_report,
    draft_fingerprint,
    file_fingerprint,
...
START_Q = 1810
END_Q = 1857
EXPECTED_BASE_END = 1809
TARGET_VERSION = "2026-09-b16"
START_NODE = 1543
END_NODE = 1546
LETTERS = "ABCDE"
QID_PATTERN = r"^Q(?:[1-9]|[1-9][0-9]{1,2}|1[0-7][0-9]{2}|18(?:[0-4][0-9]|5[0-7]))$"
IMMUTABLE_FILES = (
...
def _category_large(questions: list[dict], category_small: int) -> str:
    values = {str(row.get("category_large")) for row in questions
...
def integrate(bank_dir: Path = BANK, staging_path: Path = STAGING) -> None:
    ...
    if present and not integrated:
        raise ValueError(f"partial Lot02 allocation found: {present}")
    ...
    if not integrated:
        ...
        if report["hard_errors"] or report["warnings"]:
            raise ValueError(f"Lot02 staging validator failed: {report}")
        ...
        category_large = {
            small: _category_large(questions, small)
            for small in (8, 9, 11, 12, 14, 16, 17, 18)
        }
        ...
        if any(not all(qid in index for index in indexes) for qid in target_ids):
            raise ValueError("Lot02 is partially present across formal stores")
    ...
    if any([row["id"] for row in store] != expected_ids for store in stores):
        raise ValueError("formal stores are not a contiguous aligned Q1-Q1857 sequence")
