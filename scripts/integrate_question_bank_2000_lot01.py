"""Atomically prepare Question Bank 2000 Lot01 for formal Q1762-Q1809.

All input is validated in memory before any formal file is written.  Passing a
temporary bank directory provides the required dry-run; rerunning a completed
integration is idempotent and rejects partial allocation.
"""
...
START_Q = 1762
END_Q = 1809
EXPECTED_BASE_END = 1761
TARGET_VERSION = "2026-09-b15"
START_NODE = 1539
END_NODE = 1542
LETTERS = "ABCDE"
QID_PATTERN = r"^Q(?:[1-9]|[1-9][0-9]{1,2}|1[0-7][0-9]{2}|180[0-9])$"
IMMUTABLE_FILES = (
    "knowledge_node_canonical_map.json",
    "strong_different_question_pairs.json",
)
NEW_NODE_PREREQUISITES = {
    "L01-C9-N01": ["末梢性前庭眼振は視覚固視で抑制されやすい。"],
    "L01-C16-N01": ["アキレス腱の連続性と適切な長さは足関節底屈筋力と踵上げ機能に関与する。"],
    "L01-C17-N01": ["評価尺度の測定特性には信頼性、妥当性、反応性がある。"],
    "L01-C18-N01": ["運動観察療法は他者の動作観察と自己の動作実行を組み合わせる。"],
}
...
def _accepted_drafts(staging_path: Path = STAGING) -> list[dict]:
    ...
    if len(drafts) != END_Q - START_Q + 1:
        raise ValueError(f"expected 48 accepted drafts, got {len(drafts)}")
    if len({row.get("draft_id") for row in drafts}) != len(drafts):
...
def _category_large(tags: list[dict], category_small: int) -> str:
    values = {str(row.get("category_large")) for row in tags
              if int(row.get("category_small")) == category_small}
    if len(values) != 1:
...
def integrate(bank_dir: Path = BANK, staging_path: Path = STAGING) -> None:
    ...
    if present and not integrated:
        raise ValueError(f"partial Lot01 allocation found: {present}")
    ...
    new_node_by_draft = {
        row["draft_id"]: f"KN{START_NODE + offset:04d}"
        for offset, row in enumerate(new_drafts)
    }
    ...
    if not integrated:
        ...
        if report["hard_errors"] or report["warnings"]:
            raise ValueError(f"Lot01 staging validator failed: {report}")
        ...
        category_large = {
            small: _category_large(questions, small) for small in (9, 16, 17, 18)
        }
        for offset, draft in enumerate(drafts):
            ...
            if is_new:
                ...
                prerequisites = list(NEW_NODE_PREREQUISITES[draft["draft_id"]])
                ...
                if any(canonicalize_knowledge_node_id(tag.get("knowledge_node_id")) != node_id
            ...
            answers.append({
                "id": qid, "display_answer": correct[0],
                "accepted_answer_sets": [correct], "answer_basis": "LT_original",
            })
            explanations.append({
                "id": qid, "explanation": draft["explanation"],
                "choice_explanations": _choice_map(draft["choice_explanations"]),
            })
            tags.append({
                "id": qid, "theme": draft.get("title"),
                "knowledge_node": node["label"], "knowledge_node_id": node_id,
                "task": draft["proposed_task"], "primary_ability": draft["primary_ability"],
        ...
        if any(not all(qid in index for index in indexes) for qid in target_ids):
            raise ValueError("Lot01 is partially present across formal stores")
    ...
    if any([row["id"] for row in store] != expected_ids for store in stores):
        raise ValueError("formal stores are not a contiguous aligned Q1-Q1809 sequence")
