"""Atomically integrate sealed Question Bank 2000 Lot03 as Q1858-Q1905.
...
TARGET_VERSION = "2026-09-b17"
START_NODE = 1547
END_NODE = 1551
LETTERS = "ABCDE"
QID_PATTERN = r"^Q(?:[1-9]|[1-9][0-9]{1,2}|1[0-8][0-9]{2}|190[0-5])$"
...
def integrate(bank_dir: Path = BANK, staging_path: Path = STAGING) -> None:
    ...
    if present and not integrated:
        raise ValueError(f"partial Lot03 allocation found: {present}")

    new_drafts = [row for row in drafts if row.get("slot_type") == "new_node"]
    if len(new_drafts) != END_NODE - START_NODE + 1:
        raise ValueError(f"expected five new Node drafts, got {len(new_drafts)}")
    ...
    if not integrated:
        ...
        if report["hard_errors"] or report["warnings"]:
            raise ValueError(f"Lot03 staging validator failed: {report}")
        ...
        if any(not all(qid in index for index in indexes) for qid in target_ids):
            raise ValueError("Lot03 is partially present across formal stores")
    ...
    if any([row["id"] for row in store] != expected_ids for store in stores):
        raise ValueError("formal stores are not a contiguous aligned Q1-Q1905 sequence")
