...
2. `reports/question_bank_2000_lot01_targets_v01.json`
3. `reports/question_bank_2000_lot01_assignment_v01.json`
4. `staging/question_bank_2000_lot01_template_v01.json`
...
Use the 48 template draft IDs. Fill medically sound PT national-exam-level content, metadata, evidence, semantic reviews, and new-Node collision reviews. The assignment JSON is a **suggestion**, not permission to force an implausible task/level/Safety assignment. If an assignment is medically poor, change assignments across drafts while preserving the exact final lot quotas enforced by the validator.
...
7. Run `python reports/question_bank_2000_lot01_seal.py` only after content/reviews are complete. The seal tool must not be used to approve unfinished content.
8. Run `python reports/question_bank_2000_lot01_validate.py` and require `hard_errors=[]`.
9. Run the relevant Lot01/production tests, Question Bank validator, schema/manifest checker, full CI-equivalent pytest, and `git diff --check`.
10. Commit/push to the existing branch `analysis/qb2000-remaining-allocation-v01` / PR #268. Keep PR Draft and do not merge.
