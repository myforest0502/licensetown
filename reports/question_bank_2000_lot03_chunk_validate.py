"""Fail-closed authoring-completeness validator for one Lot03 8-draft chunk."""
...
def build_report(payload: dict) -> dict:
    ...
    check(payload.get("lot") == "question_bank_2000_production_lot03_v01", "lot id mismatch")
    check(payload.get("status") == "completed_chunk", "chunk status must be completed_chunk")
    check(payload.get("formal_baseline") == "Q1-Q1857", "formal baseline mismatch")
