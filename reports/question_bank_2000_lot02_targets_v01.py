"""Select deterministic existing-Node targets for Question Bank 2000 production Lot02.
...
CATEGORY_SLOT = {
    ...
    11: {"singleton": 3, "multi": 1, "new": 1},
    12: {"singleton": 3, "multi": 1, "new": 1},
    14: {"singleton": 2, "multi": 1, "new": 1},
    16: {"singleton": 4, "multi": 1, "new": 0},
...
def main() -> int:
    ...
    if max(map(qnum, questions)) != FORMAL_BASELINE_END:
        raise ValueError(
            f"Lot02 selector requires formal Q1-Q{FORMAL_BASELINE_END}; "
    ...
    if category_question_totals != expected_category_totals:
        raise ValueError(f"Lot02 category totals mismatch: {category_question_totals}")
