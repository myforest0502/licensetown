"""Build formal reference packets for any Lot02 authoring chunk.
...
OUT_DIR = ROOT / "reports" / "question_bank_2000_lot02_reference_packets"
...
def build(chunk_no: int) -> dict:
    ...
    if chunk.get("formal_baseline") != "Q1-Q1809":
        raise ValueError("Lot02 reference packet requires formal baseline Q1-Q1809")
    qids: list[str] = []
    ...
    if max(int(qid[1:]) for qid in stores["question"]) != 1809:
        raise ValueError("formal Question Bank is no longer Q1-Q1809")
    packet = []
    for qid in qids:
    ...
    return {"chunk": chunk_no, "formal_baseline": "Q1-Q1809", "references": packet}
