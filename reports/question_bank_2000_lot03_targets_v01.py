"""Select deterministic existing-Node targets for Question Bank 2000 production Lot03.
...
FORMAL_BASELINE_END = 1857
RECENT_EXCLUDE_AFTER = 1809
EXCLUDE_NODE = "KN0779"

# Lot03 takes 48 of the 137 remaining original questions. Category totals are a
# largest-remainder allocation of the live post-Lot02 gap. Five new Nodes are
# reserved across underfilled domains; the actual singleton/multi split is solved
# against live inventory while preserving exactly 30/13/5.
CATEGORY_SLOT = {
    1: {"singleton": 2, "multi": 1, "new": 1},
    2: {"singleton": 4, "multi": 1, "new": 1},
    3: {"singleton": 1, "multi": 0, "new": 0},
    4: {"singleton": 2, "multi": 1, "new": 0},
    5: {"singleton": 1, "multi": 0, "new": 0},
    6: {"singleton": 2, "multi": 1, "new": 0},
    7: {"singleton": 2, "multi": 1, "new": 1},
    8: {"singleton": 4, "multi": 1, "new": 1},
    9: {"singleton": 1, "multi": 0, "new": 0},
    10: {"singleton": 2, "multi": 1, "new": 0},
    11: {"singleton": 1, "multi": 1, "new": 0},
...
TARGET_MULTI_TOTAL = 13
TARGET_SINGLETON_TOTAL = 30
TARGET_NEW_TOTAL = 5
...
def _solve_slot_split(inventory: dict) -> dict[int, dict[str, int]]:
    ...
    if solution is None:
        ...
        raise ValueError(f"no feasible Lot03 30/13 Node-slot allocation: {detail}")
...
def main() -> int:
    ...
    if max(map(qnum, questions)) != FORMAL_BASELINE_END:
        raise ValueError(
            f"Lot03 selector requires formal Q1-Q{FORMAL_BASELINE_END}; "
    ...
    for node_id, qids in groups.items():
        ...
        demands = sorted(
            {(str(tags[qid]["task"]), str(tags[qid]["primary_ability"])) for qid in qids}
        )
        ...
        record = {
            ...
            "question_ids": qids,
            "existing_demands": [
                {"task": task, "primary_ability": ability} for task, ability in demands
            ],
        ...
        if len(qids) == 1:
            ...
            record["rank_score"] = list(score)
            record["selection_intent"] = (
                "add a demand outside the existing demand set; weak multi Nodes are preferred"
            )
    ...
    for category, quota in actual_slot.items():
        ...
        for kind in ("singleton", "multi"):
            ...
            for index, record in enumerate(picks, start=1):
                ...
                row["lot_target_id"] = f"L03-C{category}-{kind[:1].upper()}{index:02d}"
                selected.append(row)
    ...
    if len(selected) != 43 or len({row["canonical_node_id"] for row in selected}) != 43:
        raise ValueError("Lot03 existing-Node selection must be 43 unique canonical Nodes")
    slot_counts = Counter(row["slot_type"] for row in selected)
    if slot_counts != Counter({"singleton_second": 30, "multi_reinforcement": 13}):
    ...
    if category_question_totals != expected_category_totals:
        raise ValueError(f"Lot03 category totals mismatch: {category_question_totals}")
    ...
    output = {
        ...
        "required_existing_targets": 43,
        "required_new_nodes": 5,
        "required_total_questions": 48,
        "required_strong_formations": 41,
        "required_safety_augment": 13,
        "guardrails": [
            "No Q IDs are allocated by this target roster.",
            "No Knowledge Node registry write is performed.",
            "Nodes receiving Lot02 supply after Q1809 are not immediately retargeted.",
            "Per-category singleton/multi split may rebalance only when formal inventory requires it; global 30/13 and category totals remain exact.",
    ...
    out = ROOT / "reports" / "question_bank_2000_lot03_targets_v01.json"
