"""Fail-closed authoring-completeness validator for one Lot01 8-draft chunk.

This is intentionally weaker than the final 48-question Lot01 validator: it checks
only chunk-local authoring completeness and invariants. Passing a chunk never
permits integration, Q-ID allocation, Node-ID allocation, sealing, or formal writes.
"""
...
from reports.question_bank_2000_lot01_validate import TASK_PRIMARY, normalize
...
def build_report(payload: dict) -> dict:
    ...
    check(payload.get("lot") == "question_bank_2000_production_lot01_v01", "lot id mismatch")
    check(payload.get("status") == "completed_chunk", "chunk status must be completed_chunk")
    check(payload.get("formal_baseline") == "Q1-Q1761", "formal baseline mismatch")
    for key in ("q_ids_reserved", "production_write", "db_write"):
        check(payload.get(key) is False, f"{key} must be false")

    drafts = payload.get("drafts", [])
    check(isinstance(drafts, list) and len(drafts) == 8, f"chunk must contain exactly 8 drafts, got {len(drafts) if isinstance(drafts, list) else 'non-list'}")
