"""Create a quota-exact suggested demand/level/Safety assignment for Lot02.
...
suggestions while preserving all final Lot02 quotas and validator rules.
"""
...
OUT = ROOT / "reports" / "question_bank_2000_lot02_assignment_v01.json"
...
EVAL_WORDS = (
    "評価", "検査", "尺度", "スコア", "測定", "MMT", "筋力", "ROM", "歩行", "血圧",
    "反射", "発達", "認知", "心理", "運動学",
)
DEVICE_WORDS = (
    "装具", "車椅子", "義足", "杖", "歩行器", "電気刺激", "機器", "足部", "スプリント",
)
INTERVENTION_WORDS = (
    "治療", "訓練", "運動", "介助", "管理", "予防", "除圧", "調整", "指導", "リハ",
    "療法", "練習", "環境調整",
)
PROGNOSIS_WORDS = (
    "予後", "回復", "骨癒合", "再発", "進行", "重症度", "転帰", "発達", "自然経過",
)
SAFETY_WORDS = (
    "過反射", "骨折", "脱臼", "心停止", "呼吸", "低酸素", "出血", "感染", "塞栓", "転倒",
    "危険", "中止", "緊急", "けいれん", "てんかん", "脊髄損傷", "胸痛", "失神", "自殺",
)
FINDING_WORDS = (
    "所見", "特徴", "症状", "徴候", "障害", "病変", "麻痺", "眼振", "感覚", "疼痛", "筋萎縮",
    "発達", "認知", "行動", "心理", "歩容", "運動連鎖",
)
GOAL_WORDS = (
    "ADL", "生活", "復帰", "目標", "活動", "参加", "学校", "就労", "家族", "役割",
)

CATEGORY_TASK_BONUS = {
    8: {"finding_interpretation": 2, "safety_priority": 2, "prognosis_prediction": 1},
    9: {"finding_interpretation": 2, "safety_priority": 1, "prognosis_prediction": 1},
    ...
    16: {"device_selection": 2, "safety_priority": 1, "prognosis_prediction": 1},
    17: {"assessment_selection": 5, "finding_interpretation": 1},
    18: {"intervention_selection": 5, "device_selection": 1, "functional_goal_decision": 1},
}
...
def allowed_tasks(row: dict) -> list[str]:
    existing = {
        (str(x["task"]), str(x["primary_ability"])) for x in row.get("existing_demands", [])
    }
    return [
        task for task, ability in TASK_PRIMARY.items() if (task, ability) not in existing
    ]


def main() -> int:
    roster = read(ROSTER)
    if roster.get("formal_baseline") != "Q1-Q1809":
        raise ValueError(f"unexpected Lot02 baseline: {roster.get('formal_baseline')}")
    ...
    for reservation in roster["new_node_reservations"]:
        ...
        for index in range(1, count + 1):
            rows.append(
                {
                    "draft_id": f"L02-C{category}-N{index:02d}",
                    "slot_type": "new_node",
    ...
    order = sorted(
        range(len(rows)),
        key=lambda i: (len(allowed_tasks(rows[i])), rows[i]["is_new"], rows[i]["draft_id"]),
    )
    ...
    safety_task_indices = [
        i for i in range(len(rows)) if assignment_by_idx[i] == "safety_priority"
    ]
    risk_candidates = []
    for i, row in enumerate(rows):
        ...
        category_risk = row["category_small"] in {8, 9, 11, 16, 18}
        risk_candidates.append((risk, category_risk, -i, i))
    extra_needed = SAFETY_AUGMENT - len(safety_task_indices)
    extra_safety = {
        item[-1] for item in sorted(risk_candidates, reverse=True)[:extra_needed]
    }
    ...
    for i, row in enumerate(rows):
        ...
        safety = (
            ...
            else ("moderate" if i in extra_safety else "none")
        )
        output_rows.append(
            {
                **row,
    ...
    safety_count = sum(
        r["suggested_safety"] in {"moderate", "critical"} for r in output_rows
    )
    ...
    if strong_structural != 44:
        raise ValueError(
            f"all 44 existing targets should have a structurally distinct demand, got {strong_structural}"
        )

    category_counts = Counter(r["category_small"] for r in output_rows)
    expected_categories = Counter({8: 8, 9: 6, 11: 5, 12: 5, 14: 4, 16: 5, 17: 7, 18: 8})
        ...
        json.dumps(
            {
                ...
                for k in (
                    ...
                    "task_counts",
                    "level_counts",
                    "safety_moderate_or_critical",
                    "structurally_distinct_existing_assignments",
                )
            },
            ensure_ascii=False,
            indent=2,
