"""Build the structural authoring template for Question Bank 2000 production Lot02."""
...
TARGETS = ROOT / "reports" / "question_bank_2000_lot02_targets_v01.json"
OUT = ROOT / "staging" / "question_bank_2000_lot02_template_v01.json"
TASK_QUOTA = {
...
def main() -> int:
    ...
    if len(drafts) != 48 or len({d["draft_id"] for d in drafts}) != 48:
        raise ValueError("Lot02 authoring template must contain 48 unique drafts")
    ...
    payload = {
        ...
        "authoring_rules": [
            ...
            "New-Node slots require semantic search against formal Q1-Q1809 before a Node label is proposed.",
            "Every accepted draft requires evidence URLs/support and a completed semantic_review before sealing.",
            "AI review must keep expert_signoff=false unless a real human expert signs off.",
        ],
    }
    OUT.write_text(json.dumps(payload, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
    print(json.dumps({"drafts": 48, "existing": 44, "new_node": 4, "output": str(OUT.relative_to(ROOT))}, ensure_ascii=False))
    return 0
