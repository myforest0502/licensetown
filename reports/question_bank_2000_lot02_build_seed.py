"""Build a non-final Lot02 authoring seed by combining template and quota-exact suggestions."""
...
TEMPLATE = ROOT / "staging" / "question_bank_2000_lot02_template_v01.json"
ASSIGNMENT = ROOT / "reports" / "question_bank_2000_lot02_assignment_v01.json"
OUT = ROOT / "staging" / "question_bank_2000_lot02_authoring_seed_v01.json"
...
def main() -> int:
    ...
    if template.get("formal_baseline") != "Q1-Q1809" or assignment.get("formal_baseline") != "Q1-Q1809":
        raise ValueError("Lot02 template/assignment baseline must be Q1-Q1809")
    ...
    output = {
        **{k: v for k, v in template.items() if k != "drafts"},
        ...
        "drafts": drafts,
        "seed_warning": (
            "Suggested task/level/Safety values are quota-exact structural aids only. "
            "Medical authoring may change them if final Lot02 validator quotas remain exact."
        ),
    }
    if len(drafts) != 48 or len({d["draft_id"] for d in drafts}) != 48:
        raise ValueError("Lot02 authoring seed must contain 48 unique drafts")
