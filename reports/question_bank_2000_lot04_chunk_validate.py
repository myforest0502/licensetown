"""Fail-closed authoring-completeness validator for one Lot04 8-draft chunk."""
...
def build_report(payload: dict) -> dict:
    ...
    check(payload.get("lot") == "question_bank_2000_production_lot04_v01", "lot id mismatch")
    check(payload.get("status") == "completed_chunk", "chunk status must be completed_chunk")
    check(payload.get("formal_baseline") == "Q1-Q1905", "formal baseline mismatch")
