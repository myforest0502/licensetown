...
def test_lot04_integrator_dry_run_is_atomic_and_idempotent(tmp_path):
    ...
    assert len(nodes) == len(node_index) == 1555
    assert all(f"KN{i:04d}" in node_index for i in range(1552, 1556))

    tags = {row["id"]: row for row in stores[3]}
    new_drafts=[(offset,row) for offset,row in enumerate(read(STAGING)["drafts"]) if row["slot_type"]=="new_node"]
    ...
    # Existing-node Lot04 additions must create a different metadata demand.
