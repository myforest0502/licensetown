...
def test_site_wrapper_uses_completed_responsive_contract():
    ...
    assert "20260824-responsive1" in html
    assert 'id="pc-view"' in html
    assert 'id="mobile-view"' in html
    assert 'data-source="/site/source/pc"' in html
    assert 'data-source="/site/source/mobile"' in html
    assert 'data-base="/site/preview-pc/"' in html
    assert 'data-base="/site/preview-724/"' in html
...
def test_site_keeps_724_canvas_scaling_and_pc_mobile_switch():
    ...
    assert "frame.srcdoc=html" in js
