...
def test_all_1528_canonical_nodes_are_classified_and_counts_balance():
    records = build_repairability_audit()
    summary = summarize_repairability(records)
    assert summary["canonical_node_count"] == 1528
    tags = json.loads(
    ...
    assert summary["singleton_node_count"] + summary["multi_question_node_count"] == 1528
    ...
    assert summary["weak_alt_question_only_node_count"] == 22
