...
def test_formal_question_bank_passes_schema_and_cross_file_validation():
    ...
    assert report["knowledge_node_id_present"] == 1680
    ...
    assert report["registry_confirmed_shared_groups"] == 129
    assert report["registry_confirmed_shared_questions"] == 271
    assert report["registry_singleton_nodes"] == 1409
...
def test_registry_allows_confirmed_shared_ids_and_maps_every_question_once():
    ...
    assert len(shared) == 129
    assert sum(len(node["question_ids"]) for node in shared) == 271
    assert all(len(node["question_ids"]) >= 2 for node in shared)
    assert len(mapped_questions) == len(set(mapped_questions)) == 1680
